...
系统功能模块划分如图~\ref{fig:module-structure} 所示。各模块之间通过清晰的接口边界与数据流向协同工作：前端负责交互采集与结果展示，后端负责认证、画像、推荐与日志处理，推荐引擎负责核心排序计算，数据库负责统一持久化。该分层结构既保证了业务职责的解耦，也便于后续功能扩展与维护。

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{images/系统功能模块图.png}
\caption{系统功能模块图}
...
系统详细设计在概要设计的基础上，进一步说明前端交互、推荐流程与数据闭环的实现路径。系统总体流程如图~\ref{fig:overall-flow} 所示，展示了从用户注册登录、画像完善、问答提交到推荐返回、结果确认与历史记录沉淀的完整闭环。

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{images/总体流程图.png}
\caption{系统总体流程图}
...
历史记录模块在用户确认选择后触发。后端接收批次 ID 与菜品 ID，查询该批次对应的曝光日志以验证请求合法性，随后将用户 ID、菜品 ID、批次 ID 及问答快照写入历史记录表。若用户选择推荐列表首条菜品，则标记为首条采纳，便于后续评估推荐准确率。

交互日志模块记录推荐全链路中的关键行为节点。在推荐阶段，系统将批次ID、用户ID与Top-20推荐列表以曝光类型写入日志；在用户浏览阶段，前端异步上报点击行为；在确认阶段，后端将确认行为写入日志，并将推荐列表中未被选择的菜品作为负样本写入日志。通过同一批次ID关联曝光、点击、确认与负样本记录，形成完整的交互闭环。
...
向量生成服务封装 sentence-transformers 模型的加载与编码逻辑，提供菜品向量生成与用户向量融合两个核心函数：菜品向量通过拼接名称、菜系、口味标签、描述、食材及城市等字段生成语义文本，再经模型编码为 768 维向量；用户向量则融合画像偏好与问答场景文本进行编码。
...
菜品管理模块实现店铺与菜品信息的录入、查询及语义向量生成。后端在菜品录入时同步触发向量化流程：将菜品的名称、菜系、口味标签、描述、食材及城市按固定模板拼接为语义文本，经 sentence-transformers 编码为 768 维向量后回写至数据库。若模型不可用，服务返回 503 状态码并拒绝写入，防止无效向量进入候选池。前端菜品录入页面向管理员或授权用户提供店铺与菜品信息表单。

历史记录与数据闭环模块实现用户选择行为的持久化存储与全链路交互日志记录。后端历史查询接口按时间倒序返回用户过往选择，支持关联查询菜品详情与问答快照。日志记录接口在推荐生成、用户点击与选择确认三个关键节点写入结构化数据，同一批次通过 UUID 关联曝光、确认与负样本记录。前端历史记录页按时间倒序展示过往选择，用户可查看详情或基于历史问答快照重新发起推荐。
